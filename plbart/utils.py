    ...
    if os.path.exists(cache_fn) and not is_sample:
        ...
        for task in task_list:
            if task == 'summarize':
                ...
                sub_tasks = ['java-java', 'python-python']
            elif task == 'refine':
...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'concode':
        ...
        # using this thing
        if sub_task in ['java-java', 'python-python']:
            src, tgt = sub_task.split('-')
            train_fn = '{}/src_train.{}.{},{}/tgt_train.{}.{}'.format(data_root, sub_task, src, data_root,sub_task,tgt)
            dev_fn = '{}/src_valid.{}.{},{}/tgt_valid.{}.{}'.format(data_root, sub_task, src, data_root,sub_task,tgt)
            test_fn = '{}/src_test.{}.{},{}/tgt_test.{}.{}'.format(data_root, sub_task, src, data_root,sub_task,tgt)
        else:
            data_dir = '{}/{}'.format(data_root, task)
            if sub_task == 'cs-java':
